...
export default function ProductCategoriesPage() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      if (res.success && Array.isArray(res.data)) {
        ...
        if (!hasUncategorized) {
          const defaultUncategorized: ProductCategory = {
            id: "uncategorized",
            name: "Uncategorized",
            slug: "uncategorized",
          ...
          list = [defaultUncategorized, ...list];
        }
        setCategories(list);
      }
    } catch (err: unknown) {
      const apiErr = err as ApiError;
      addToast("error", apiErr?.message || "Failed to load product categories.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Filter Categories list based on search query
  ...
  const handleCreateOrUpdate = async (e: React.FormEvent) => {
    ...
    if (editingCategory) {
      ...
      const duplicate = categories.find(
        (c) => c.slug === finalSlug && c.id !== editingCategory.id
      );
      if (duplicate) {
        addToast("error", "A category with this slug already exists.");
        return;
      }
      setSubmitting(true);
      try {
        const res = await updateProductCategory(editingCategory.id, {
          name: name.trim(),
          slug: finalSlug,
    ...
    // Create path
    const duplicate = categories.find((c) => c.slug === finalSlug);
    if (duplicate) {
      addToast("error", "A category with this slug already exists.");
      return;
    }
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      // Reassign products to "uncategorized" locally
      setCategories((prev) => {
        const remaining = prev.filter((c) => c.id !== deletingCategory.id);
        return remaining.map((c) =>
          c.slug === "uncategorized"
            ? {
                ...c,
                _count: {
                  products: (c._count?.products ?? 0) + (deletingCategory._count?.products ?? 0),
                },
              }
            : c
        );
  ...
  return (
    <div className="min-h-full p-8 space-y-6 font-sans relative">
      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-zinc-900 tracking-tight">Product Categories</h1>
          <p className="text-sm text-zinc-500 mt-0.5 font-medium">Manage categories for your E-Commerce Store</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {loading ? (
          ...
          <>
            ...
            <div className="lg:col-span-7 bg-white rounded-3xl p-6 border border-zinc-100 shadow-sm space-y-5">
              <h2 className="text-base font-extrabold text-zinc-800">All Categories</h2>
                      ...
                      className={`flex items-center justify-between py-3.5 px-3 rounded-2xl transition-all duration-200 group
                        ${
                          editingCategory?.id === cat.id
                            ? "bg-[#FFF0F2]/50 border border-[#FFF0F2]"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <span className="bg-[#FFF0F2] border border-[#FFF0F2] text-[#B31046] text-[10px] font-extrabold px-3 py-1 rounded-full whitespace-nowrap">
                          {cat._count?.products ?? 0} {(cat._count?.products ?? 0) === 1 ? "product" : "products"}
                        </span>
                      </div>
                    </div>
                  ))
            ...
            <div className="lg:col-span-5 bg-white rounded-3xl p-6 border border-zinc-100 shadow-sm space-y-5">
              ...
              <form onSubmit={handleCreateOrUpdate} className="space-y-4">
                {/* Name */}
                <div>
                  <label htmlFor="cat-name" className="text-xs font-extrabold text-zinc-800 tracking-wide block mb-1.5">
                    Category name
                  </label>
                  <input
                    id="cat-name"
                    ...
                    className="w-full bg-[#FFF0F2]/30 border border-[#FFF0F2]/50 hover:bg-[#FFF0F2]/45 focus:bg-white focus:border-[#B31046] focus:ring-2 focus:ring-[#B31046]/5 px-4 py-3.5 rounded-2xl text-sm font-semibold text-zinc-800 placeholder-zinc-400 outline-none transition-all"
                  />
                </div>

                {/* Slug */}
                <div>
                  <label htmlFor="cat-slug" className="text-xs font-extrabold text-zinc-800 tracking-wide block mb-1.5">
                    Slug
                  </label>
                  <input
                    id="cat-slug"
                    ...
                    className="w-full bg-[#FFF0F2]/30 border border-[#FFF0F2]/50 hover:bg-[#FFF0F2]/45 focus:bg-white focus:border-[#B31046] focus:ring-2 focus:ring-[#B31046]/5 px-4 py-3.5 rounded-2xl text-sm font-semibold text-zinc-800 placeholder-zinc-400 outline-none transition-all"
                  />
                  <p className="text-[10px] text-zinc-400 leading-normal mt-1.5 font-medium">
                ...
                <div>
                  <div className="flex justify-between items-center mb-1.5">
                    <label htmlFor="cat-desc" className="text-xs font-extrabold text-zinc-800 tracking-wide">
                      Description
                    </label>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm border border-zinc-100 shadow-2xl relative space-y-5 animate-in fade-in zoom-in-95 duration-200 text-center">
            ...
            {(deletingCategory._count?.products ?? 0) > 0 && (
              <div className="bg-[#FFF0F2]/40 border border-[#FFF0F2]/60 rounded-2xl p-4 text-left">
                <p className="text-xs font-bold text-[#B31046] leading-normal">
                  ⚠️ This category contains {deletingCategory._count?.products ?? 0}{" "}
                  {(deletingCategory._count?.products ?? 0) === 1 ? "product" : "products"}. Deleting it will automatically
                  reassign all of them to "Uncategorized".
                </p>
              </div>
            )}
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                disabled={deleting}
                className="flex-1 py-3 border border-zinc-200 text-zinc-650 hover:bg-zinc-50 font-bold text-sm rounded-full transition-all active:scale-[0.98] select-none cursor-pointer disabled:opacity-40"
              >
                Cancel
              </button>
              <button
